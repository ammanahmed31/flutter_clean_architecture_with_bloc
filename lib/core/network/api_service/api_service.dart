...
class ApiService implements ApiInterface {
  ...
  Future<List<T>> getCollectionData<T>({
    ...
    try {
      ...
      final data = await _dioService.get<List<Object?>>(
        endpoint: endpoint,
        cacheOptions: _dioService.globalCacheOptions?.copyWith(policy: cachePolicy, maxStale: cacheAgeDays != null ? Duration(days: cacheAgeDays) : null),
  ...
  /// The default value is `true`.
  @override
  Future<T> postData<T>({required String endpoint, required JSON data, CancelToken? cancelToken, bool requiresAuthToken = true, required T Function(ResponseModel<JSON> response) converter}) async {
    ResponseModel<JSON> response;

    try {
      // Entire map of response
      response = await _dioService.post<JSON>(endpoint: endpoint, data: data, options: Options(extra: <String, Object?>{'requiresAuthToken': requiresAuthToken}), cancelToken: cancelToken);
    } on Exception catch (ex) {
      throw CustomException.fromDioException(ex);
    }
  ...
  /// The default value is `true`.
  @override
  Future<T> patchData<T>({required String endpoint, required JSON data, CancelToken? cancelToken, bool requiresAuthToken = true, required T Function(ResponseModel<JSON> response) converter}) async {
    ResponseModel<JSON> response;

    try {
      // Entire map of response
      response = await _dioService.patch<JSON>(endpoint: endpoint, data: data, options: Options(extra: <String, Object?>{'requiresAuthToken': requiresAuthToken}), cancelToken: cancelToken);
    } on Exception catch (ex) {
      throw CustomException.fromDioException(ex);
    }
  ...
  /// The default value is `true`.
  @override
  Future<T> deleteData<T>({required String endpoint, JSON? data, CancelToken? cancelToken, bool requiresAuthToken = true, required T Function(ResponseModel<JSON> response) converter}) async {
    ResponseModel<JSON> response;

    try {
      // Entire map of response
      response = await _dioService.delete<JSON>(endpoint: endpoint, data: data, options: Options(extra: <String, Object?>{'requiresAuthToken': requiresAuthToken}), cancelToken: cancelToken);
    } on Exception catch (ex) {
      throw CustomException.fromDioException(ex);
    }
